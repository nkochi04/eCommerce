...
namespace DesktopAppAPI
{
    public class SeedData
    {
        ...
        private static void InitializeDepartments(desktopAppDbContext _db)
        {
            ...
            {
                _db.Departments.Add(new Department { ID = Guid.NewGuid(), Name = name, Payment_Adress = random.Next(1000, 10000).ToString() });
            }
        }
        ...
        private static void InitializeUsers(desktopAppDbContext _db)
        {
            ...
            {
                _db.Users.Add(new User { ID = Guid.NewGuid(), Username = $"admin{i}", Password = "admin", Firstname = "admin", Lastname = "admin", Department = department });
                i++;
            }
            _db.Users.Add(new User { ID = Guid.NewGuid(), Username = $"nkochi", Password = "admin", Firstname = "admin", Lastname = "admin", Department = _db.Departments.Where(x => x.Name == "Management").SingleOrDefault() });
        }
        ...
        private static void InitializeProducts(desktopAppDbContext _db)
        {
            ...
            List<Seller> seller = [.. _db.Sellers];
        ...
        private static void InitializePieces(desktopAppDbContext _db)
        {
            ...
            foreach (var product in _db.Products)
            {
                for (int i = 0; i < 10; i++)
                {
                    ...
                    _db.Add(new Piece { Product = product, Serial_Number = randomNumber });
                }
            }
        }
